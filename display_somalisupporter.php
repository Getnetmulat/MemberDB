...
if (login_check($mysqli) == true) : 
	...
	elseif($_GET['field']=='Field_of_Job')
		   $field="Field_of_Job";
	elseif($_GET['field']=='Support_Party')
		   $field="Support_Party";
    elseif($_GET['field']=='Supporter_Status')
		   $field="Supporter_Status";
		elseif($_GET['field']=='Supporter_Year')
		   $field="Supporter_Year";
	}
	...
	$sql = "SELECT *
				FROM eprdf_supporter where Support_Party in('Somali','ሶማሌ')    
				ORDER BY $field $sort 
	...
	$sqlcount = "	SELECT COUNT(ID) 
					AS numrows 
					FROM eprdf_supporter where Support_Party in('Somali','ሶማሌ') ";// Find the number of rows in the query
	...
	echo "	<tr> 
				...
				<th nowrap width='3%'><a href='$phpself?sorting=$sort&field=ID&page=$pageNum'>ID </a></th>
				...
				<th nowrap width='17%'><a href='$phpself?sorting=$sort&field=Education&page=$pageNum'>Education</a></th>
				<th nowrap width='15%'><a href='$phpself?sorting=$sort&field=Field_Of_Study&page=$pageNum'>Field Of Study</a></th>
				<th nowrap width='15%'><a href='$phpself?sorting=$sort&field=Field_of_Job&page=$pageNum'>Field Of Job</a></th>
				<th nowrap width='25%'><a href='$phpself?sorting=$sort&field=Support_Party&page=$pageNum'> Region/City Office you Support</a></th>
				<th nowrap width='8%'><a href='$phpself?sorting=$sort&field=Supporter_Status&page=$pageNum'>Supporter_Status</a></th>
				<th nowrap width='8%'><a href='$phpself?sorting=$sort&field=Supporter_Year&page=$pageNum'>Supporter Year</a></th>
	...
	If ($numberOfRows == 0) {
		...
		echo '
              <td nowrap>'.$row['ID'].'</td>
              <td>
                 <img src="data:Photo;base64,'.base64_encode($row['Photo'] ).'" alt ="" class="img-circle" style =" height :100px; width:100px;" class="img-thumnail" />  
              </td> ';               
                  ?>      
      ...
      $('#insert').click(function(){  
		...
		<?php
		   echo "
						...
						<td nowrap>".$row['Field_of_Job']."</td>
						<td nowrap>".$row['Support_Party']."</td>	
						<td nowrap>".$row['Supporter_Status']."</td>
						<td nowrap>".$row['Supporter_Year']."</td>	        
				";
    if(mysqli_num_rows($mysqli->query("SELECT `role` FROM `members` WHERE `role`='ADMIN' AND `id`=".$_SESSION['user_id'].""))!=0)
			     {
	echo "<td nowrap> <a href='edit_Supporter.php?ID=".$row['ID']."'><img src='images/b_edit.png' alt='Edit Member'></a></td>			
		<td nowrap> <a href='#?ID=".$row['ID']."'><img src='images/b_drop.png' alt='Delete Supporter' onclick='return confirm('ARE YOU SURE YOU WANT TO DELETE ".$row['ID']."?!\n Click OK to confirm!');'></a></td>
