...
<tr> 
	<td>&nbsp;</td>
	 <td height="29" colspan="3" align="center"><h3>Edit Prosperity Party Supporter's Profiles By ID:</h3></td>
</tr>
<tr>
...
$sql="select ID from Prosperity Party_supporter where Support_Party ='Amhara' ORDER BY ID";
...
$conn = mysqli_connect(HOST,USER,PASSWORD,DATABASE);
	 $conn->query("SET NAMES 'utf8'");
$select="select * from Prosperity Party_supporter where ID='$ID'";
...
<center>
  ...
  box-shadow:0 0 1px #999;
   ...
   color:#058119;
 background: #8DC28D;">
  <h2>Update Prosperity Party Supporter's Information:</h2></legend></td></tr></center>
	<tr><tr>
 </tr></tr>
<tr>
<td>&nbsp;</td>
...
$pn=$row['Phone_Number'];
$add=$row['Address'];
$fs=$row['Field_Of_Study'];
$fj=$row['Field_of_Job'];
$ed=$row['Education'];
$ex=$row['Expriance'];
$wp=$row['Work_Place'];
$Resp=$row['Resp'];
$sp=$row['Support_Party'];
$sy=$row['Supporter_Year'];
$ss=$row['Supporter_Status'];
//$Photo=$row['Photo'];	
...
$conn = mysqli_connect(HOST,USER,PASSWORD,DATABASE);
	 $conn->query("SET NAMES 'utf8'");
	  ...
	  if($count!=0)
	  {
 $update="update Prosperity Party_supporter set 
   ID ='$ID',First_Name='$fn',Middle_Name='$mn',Last_Name='$ln',
   Sex='$sex',Age='$age',Phone_Number='$pn',Email ='$email',Expriance='$ex',Education='$ed',Field_Of_Study='$fs',Field_of_Job='$fj',Resp='$Resp',Support_Party='$sp',Supporter_Year='$sy',Supporter_Status='$ss',     Work_Place='$wp',Address='$add' where ID='$ID'";
