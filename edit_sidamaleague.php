...
<table width="100%" height="0" bgcolor ="#3f879d">
 <tr>
 	...
 	<h3 align="center">የብልፅግና ፓርቲ ሴቶችና ወጣቶች ሊግ መረጃ ቋት</h3> 
...
<tr> 
	<td>&nbsp;</td>
	 <td height="29" colspan="3" align="center"><h3>Edit Dire Dawa Prosperity Party Leagues Profile By ID:</h3></td>
</tr>
<tr>
<td rowspan="3"><img src ='images/doc8.jpg' width ='90' height ='50'></td>
<td><label>Search Dire Dawa Prosperity Party Leagues</label></td>
<td>
<?php
$conn = mysqli_connect(HOST,USER,PASSWORD,DATABASE);
	 $conn->query("SET NAMES 'utf8'");
	 $sql="select ID from league where Party in ('Sidama','Sidaama','ሲዳማ') ORDER BY ID";

$result=$conn->query($sql);
	    echo "<select required name='ID'>"; 
	    echo "<option  value=''> <b>Select League Id</b> </option>\n";
...
<center><h3> Update Prosperity Party League Information</h3></center>
...
<td>&nbsp;</td>
<td>ፆታ</td>
<td><input type="text" readonly name="Sex" value='<?php echo$user['Sex'];?>' /></td>
<td>&nbsp;</td>
<td>&nbsp;</td>
<td>እድሜ</td>
...
<td>&nbsp;</td>
<td>የሊግ አባልነት</td>
<td><input type="text"  name="member" value='<?php echo$user['member'];?>' /></td>
<td>&nbsp;</td><td>&nbsp;</td>
...
$Photo = addslashes(file_get_contents($_FILES["Photo"]["tmp_name"]));
	...
	{	
	  if($count!=0)
	  {
 $update="update league set 
           ID ='$ID',First_Name='$fn',Middle_Name='$mn',
           Last_Name='$ln',Sex='$sex',Age='$age',member='$member',
          ...
          Phone_Number='$pn',Leader_Experience='$le',Expert_Experience='$ee',T_Center='$tc',T_Type='$tt',
          T_Round='$tr',T_Year='$ty',T_Grade='$tg',Amount='$Amount',Photo='$Photo' where ID='$ID'";
  $updateresult=$conn->query($update);

	if($updateresult)
	{
	echo ('<script type="text/javascript">  alert("member update successfully") </script>');
	}
	else
	{
