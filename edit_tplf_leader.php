...
<p><span id ="header_logo">Welcome:[ <b><?php echo $_SESSION['username']; ?> ]&nbsp;&nbsp;&nbsp;            You are currently logged <?php echo $logged ?>.<a href="includes/logout.php">[Log Out!]</a>. &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo date("l").", ". date("M d") .", ". date("Y");?> </b> </p>
 ...
 <tr>
	...
	if (login_check($mysqli) == true)
	     {	
			if(mysqli_num_rows($mysqli->query("SELECT `role` FROM `members`  WHERE `role`='TPLF' AND `id`=".$_SESSION['user_id']."" ))!=0 )
			     {
	 echo "
		...
		<a href ='search_Leader_more.php'>&nbsp;&nbsp;Search More</a>&nbsp;&nbsp;
		<a href ='edit_tplf_leader.php'><img src ='images/edit.png' height='25px' width='30px'>&nbsp;&nbsp;ያስተካክሉ</a>&nbsp;&nbsp;
		...
		<a href ='view_leader_doc.php'>&nbsp;&nbsp;የትምህርት መረጃ</a>
		<a href ='transfer_tplf_Leader.php'>&nbsp;&nbsp;ዝውውር</a>
       ";
   }
}
       ?>
...
$sql="select ID from leader where Party ='TPLF' ORDER BY ID";
...
<td>&nbsp;</td>
<td>የት/ት መረጃ</td>
<td><input type="file"  name="Document" value='<?php echo$user['Document'];?>' /></td>
<td>&nbsp;</td><td>&nbsp;</td>
<td>የትምህርት መስክ</td>
...
$conn = mysqli_connect(HOST,USER,PASSWORD,DATABASE);
 ...
 $update="update leader set 
           ...
           Education='$ed',Document='$dc',Field_Of_Study='$fs',
