...
if (login_check($mysqli) == true) : 
	...
	$sql = "SELECT *
				FROM member WHERE Party in('Amhara','አማራ')  
				ORDER BY $field $sort 
	...
	$sqlcount = "	SELECT COUNT(ID) 
					AS numrows 
					FROM member WHERE Party in('Amhara','አማራ')  ";// Find the number of rows in the query
	...
	echo "	<tr>
				...
				<th nowrap width='17%'><a href='$phpself?sorting=$sort&field=Education&page=$pageNum'>የት/ት ደርጃ</a></th>
				...
				<th nowrap width='25%'><a href='$phpself?sorting=$sort&field=Membership_Year&page=$pageNum'> የአባልነት ዘመን</a></th>
...
<?php
           ...
           if(mysqli_num_rows($mysqli->query("SELECT `role` FROM `members` WHERE `role`='ADMIN' AND `id`=".$_SESSION['user_id'].""))!=0)
			     {
	      echo "
			<td nowrap> <a href='edit_member.php?ID=".$row['ID']."'><img src='images/b_edit.png' alt='Edit Member'></a></td>			
						<td nowrap> <a href='#?ID=".$row['ID']."'><img src='images/b_drop.png' alt='Delete Trainee' onclick='return confirm('ARE YOU SURE YOU WANT TO DELETE ".$row['ID']."?!\n Click OK to confirm!');'></a></td>
